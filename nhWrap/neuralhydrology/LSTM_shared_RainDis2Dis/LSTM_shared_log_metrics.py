...
def main():
    ...
    model_dirs = list(runs_dir.glob("HPC_training_zscore_norm_hidden_size256_batch_size512_learning_rate0001_2812_002447"))

    for run_dir in model_dirs:
        model_name = run_dir.name
        print(f"Evaluating model: {model_name}")
        run_config = Config(run_dir / "config.yml")
        model_params = extract_model_params(run_config)
        ...
        for basin in basins:
            ...
            selected_metrics = {
                ...
                "NSE": values.get("NSE"),
                "Peak-Timing": values.get("Peak-Timing"),
            }
            log_metrics_to_csv(csv_path, model_name, basin, selected_metrics, model_params)
            print(f"Logged metrics for model {model_name}, basin {basin}")

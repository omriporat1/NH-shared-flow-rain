...
directory = os.path.dirname(f"S:/hydrolab/home/Omri_Porat/PhD/Data/IMS_IHS_flow_and_rain_corrected/")
...
def plot_data_gaps_over_time(gap_data):
    ...
    plt.title("Cumulative Data Gaps Over Time")
    plt.xlabel("Time")
...
plot_heatmap(stats_df, flow_columns, "Flow Statistics Heatmap")

# Heatmap: Rain gauge statistics
rain_columns = [col for col in stats_df.columns if 'Rain_gauge' in col]
plot_heatmap(stats_df, rain_columns, "Rain Gauge Statistics Heatmap")

# Bar Plot: Key Statistics - NaNs
key_columns = ['Flow_m3_sec_NaNs', 'Water_level_m_NaNs']
plot_bar(stats_df, key_columns, "NaNs by Basin")

key_columns = ['Flow_m3_sec_mean', 'Water_level_m_mean']
SD_columns = ['Flow_m3_sec_std', 'Water_level_m_std']
plot_bar_with_SD(stats_df, key_columns, SD_columns, "Means by Basin")

# Line Plot: Longest Data Gap
plot_line(stats_df, 'Flow_m3_sec_longest_gap', "Longest Sequence Without Data by Basin")
